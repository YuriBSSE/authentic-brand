...
function BasicFilteringGrid({ header, data, deleteSubs, reload,role }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        if (params.value == "No") {
          return (
            <p style={{ textTransform: 'capitalize', color: '#F5AF3C',marginLeft:25 }}>
              {(params.value)}
            </p>
          )
        } else {
      ...
      renderCell: (params) => {
        return (
          <div style={{ marginLeft: 'auto', marginRight: 'auto' }}>
            <Button
              onClick={() => history.push(`/subsDetail/${params.value}`)}
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div style={{ marginLeft: 'auto', marginRight: 'auto' }}>
            <Button
              disabled={params.value.subscribe?true:false}
              onClick={() =>history.push(`/editSubs/${params.value._id}`)}
              style={{ color: params.value.subscribe?'grey':'#3ADE59', borderColor: params.value.subscribe?'grey':'#3ADE59', marginRight: 5 }}
              variant="outlined"
            >
              <Edit
                fontSize="small"
    ...
    <div style={{ height: 600, width: '100%', marginTop: 20 }}>
      ...
      <DataGrid
        ...
        rows={data.docs.map(item => ({
          name: item.name,
          price: item.price,
          cycle:getCycleDura(item.cycle),
          subscribe: item.subscribe?"Yes":"No",
          id: item._id,
          action: item
      }))}
