...
function BasicFilteringGrid({ header, data,reload ,sendMail}) {
  ...
  function sendBill(data){
      sendMail(data).then((res)=>{
        if(res.status){
          toast.success("successfully sent bill")
        }else{
          console.log(res)
    ...
    <div style={{ height: 600, width: '100%', marginTop: 20 }}>
      <DataGrid
        ...
        rows={data.docs.map(item=>({
          userName:item.user[0]?(item.user[0].first_name+" "+item.user[0].last_name):"user deleted",
          price:"$"+item.price,
          remainingPrice:"$"+(item.price-(item.paidPrice?item.paidPrice:0)),
          name:item.name,
          id:item._id,
          emailData:item
        }))}
