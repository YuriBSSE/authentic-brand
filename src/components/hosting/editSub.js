...
function EditSubs({ editSubs,getCLientList,clientList,subscribetion ,getSubscribtion}) {
    ...
    const subMemo=useMemo(()=>{
        if(subscribetion._id){
            setField({...subscribetion,client:subscribetion._user,cycleType:subscribetion.cycle})
            const dTags=subscribetion.services.map(item=>({id:item._id,text:item.name}))
            setTags(dTags)
        }
    },[subscribetion])
    ...
    function onSubmitData() {
        setSubmit(true)
        if (fields.name && fields.price && tags.length>0 && fields.cycleType && fields.client) {
            setLoading(true)
            editSubs({...fields,services:tags,id:id}).then(() => {
                setModal(true)
                setLoading(false)
                setSubmit(false)
            }).catch(err => {
...
function mapStateToProps({clientList,subscribetion}){
    return {clientList,subscribetion}
}

export default connect(mapStateToProps, actions)(EditSubs);