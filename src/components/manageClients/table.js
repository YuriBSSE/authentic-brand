...
function BasicFilteringGrid({ header, data, deleteProject, reload,userId,cId }) {
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div style={{ marginLeft: 'auto', marginRight: 'auto' }}>
            <Button
              onClick={() =>history.push(`/updateProject/${params.value}`)}
    ...
    <div style={{ height: 430, width: '100%', marginTop: 20 }}>
      ...
      <DataGrid
        ...
        rows={data.docs.map(item => ({
          name: item.name,
          price: item.price,
          status: item.status==1?"running":"complete",
          id: item._id,
          action: item._id
        }))}
...
export default connect(mapStateToProps, actions)(BasicFilteringGrid)