...
function BasicFilteringGrid({ header, data, deleteSubs, reload,cId }) {
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div style={{ marginLeft: 'auto', marginRight: 'auto' }}>
            <Button
              onClick={() =>history.push(`/editSubs/${params.value}`)}
  ...
  React.useEffect(()=>{
    setLoader(true)
    reload(cId,page).then(()=>setLoader(false))
  },[page])
  return (
    <div style={{ height: 430, width: '100%', marginTop: 20 }}>
      <DeleteModel
        visible={delModal}
